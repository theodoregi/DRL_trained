...
from drl_training import env, DT, EPSILON, URDF, TARGET, SEED, MAX_EPISODE_STEPS, ROBOT

if __name__ == "__main__":

    ###
    print("Available models:")
    for i, directory in enumerate([d for d in os.listdir('models/') if d.startswith('2DOF')]):
        print(f"\t{i+1:2d}: {directory}")
    while True:
        try:
            model_number = int(input("Choose a model number: "))
            assert model_number > 0 and model_number <= len([d for d in os.listdir('models/') if d.startswith('2DOF')])
            break
        except:
    ...
    for directory in [d for d in os.listdir('models/') if d.startswith('2DOF')]:
        ...
        ###
        if model_dir not in str(training_dir):
            continue
        ###
